...
void converterHelper(Node* root, Node* &head, Node* &last){
    if(root!=NULL){
        converterHelper(root->left, head, last);
        if(head == NULL){
            head = root;
        }
        root->left = last;
        ...
        last = root;
        converterHelper(root->right, head, last);
    }
}
Node* newMethodForConversion(Node* root){
    Node* head = NULL;
    Node* last = NULL;
    converterHelper(root, head, last);
    head->left = last;
    last->right = head;
    return head;
}
...
int main()
{
    ...
    Node *head = newMethodForConversion(root);
    displayCList(head);
